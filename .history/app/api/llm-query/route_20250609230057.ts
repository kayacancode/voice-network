...
- NEVER return an empty query string

Return only valid JSON in this format:
{
  "refined_query": "string",  // Must be a non-empty string
...
Process the input and respond with appropriate JSON. Remember to NEVER return an empty query string:`;
    ...
    if (!llmResponse.refined_query || typeof llmResponse.refined_query !== 'string' || llmResponse.refined_query.trim() === '') {
      // If no valid query, use the original transcript as fallback
      llmResponse.refined_query = user_transcript.trim();
    }
