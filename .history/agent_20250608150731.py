...
pinecone.init(
    ...
    environment="gcp-starter"  # adjust if you're using a different environment
)
index = pinecone.Index(os.getenv("PINECONE_INDEX_NAME", "ai-network"))
...
async def search_contacts_function(query: str, top_k: int = 5):
    """Function that can be called by the agent to search contacts"""
    try:
        # Generate embedding for the query
        embedding_response = openai_sync.embeddings.create(
            model="text-embedding-3-small",
...
async def entrypoint(ctx: agents.JobContext):
    # Define the function for contact search
    search_contacts_fnc = agents.llm.FunctionContext(
        search_contacts_function,
        description="Search through the user's professional network contacts (LinkedIn and Instagram) by name, title, company, location, skills, or industry",
    )
    ...
    await session.generate_reply(
        instructions="Greet the user and let them know you can help them search their professional network. Ask what kind of contacts they're looking for."
    )
