  ...
  return (
    <div className="relative min-h-[85vh] bg-white flex flex-col items-center justify-center overflow-hidden">
      ...
      <div ref={containerRef} className="absolute inset-0 pointer-events-none">
        {[...Array(6)].map((_, i) => {
          // Generate unique movement patterns for each bubble
          const verticalRange = 150 + Math.random() * 100; // 150-250px vertical movement
          const horizontalRange = 30 + Math.random() * 20; // 30-50px horizontal drift
          const rotationRange = 5 + Math.random() * 5; // 5-10 degrees rotation
          const duration = 40 + Math.random() * 20; // 40-60 seconds per cycle
          const delay = Math.random() * 10; // Stagger the start of each bubble up to 10s
            ...
            <motion.div
              ...
              animate={{
                y: [
                  ...
                  Math.random() * window.innerHeight - verticalRange,
                  Math.random() * window.innerHeight,
                ],
                x: [
                  Math.random() * window.innerWidth,
                  Math.random() * window.innerWidth + (Math.random() > 0.5 ? horizontalRange : -horizontalRange),
                  Math.random() * window.innerWidth,
                ],
                rotate: [
                  0,
                  Math.random() > 0.5 ? rotationRange : -rotationRange,
                  0,
                ],
                scale: [0.95, 1, 0.95],
              }}
