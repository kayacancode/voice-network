...
Always search the actual database when asked and provide real results. Be conversational and helpful."""
    ...
    async def handle_user_input(self, user_input: str):
        ...
        # Check for LinkedIn data fetching requests
        fetch_triggers = ['fetch', 'get my', 'update', 'import', 'load', 'sync']
        linkedin_data_triggers = ['linkedin', 'connections', 'network']
        
        is_fetch_request = any(trigger in user_lower for trigger in fetch_triggers)
        is_linkedin_data = any(trigger in user_lower for trigger in linkedin_data_triggers)
        
        if is_fetch_request and is_linkedin_data:
            return await self.handle_linkedin_fetch(user_input)
        
        # Check for network statistics requests
        stats_triggers = ['stats', 'statistics', 'how many', 'network size', 'overview']
        if any(trigger in user_lower for trigger in stats_triggers) and is_linkedin_data:
            return await self.handle_network_stats()
        ...
        if is_search_query:
            # Determine if this should be a LinkedIn-specific search
            linkedin_indicators = [
                'linkedin', 'professional network', 'connections', 'first degree', 'second degree',
                'colleagues', 'professional contacts'
            ]
            
            is_linkedin_search = any(indicator in user_lower for indicator in linkedin_indicators)
            
            # Extract search terms - remove common words
            stop_words = {'find', 'show', 'me', 'who', 'are', 'the', 'do', 'i', 'know', 'any', 'people', 'someone', 'anyone', 'contacts', 'from', 'at', 'in', 'my', 'network', 'linkedin', 'professional'}
            ...
            if search_terms:
                ...
                if is_linkedin_search:
                    # Use LinkedIn-specific search
                    include_second_degree = 'second degree' in user_lower or 'extended network' in user_lower
                    results = await self.linkedin_agent.search_linkedin_network(
                        query, 
                        limit=10, 
                        include_second_degree=include_second_degree
                    )
                    return self.linkedin_agent.format_linkedin_results(results, query)
                else:
                    # Use general contact search
    ...
    async def handle_linkedin_fetch(self, user_input: str):
        """Handle LinkedIn connection fetching requests"""
        user_lower = user_input.lower()
        
        # Determine parameters based on user input
        max_connections = 500  # default
        include_second_degree = 'second degree' in user_lower or 'extended' in user_lower
        
        # Extract number if specified
        import re
        numbers = re.findall(r'\d+', user_input)
        if numbers:
            max_connections = min(int(numbers[0]), 2000)  # Cap at 2000 for safety
        
        success, message = await self.linkedin_agent.fetch_linkedin_connections(
            max_connections=max_connections,
            include_second_degree=include_second_degree
        )
        
        if success:
            return f"Great! I've started fetching your LinkedIn connections. {message} This may take a few minutes to complete. You can search your network once it's done!"
        else:
            return f"I had trouble fetching your LinkedIn connections: {message}. Make sure your LinkedIn credentials are set up in the environment variables."
    
    async def handle_network_stats(self):
        """Handle network statistics requests"""
        stats = await self.linkedin_agent.get_network_stats()
        
        if not stats:
            return "I couldn't retrieve your network statistics. Make sure your LinkedIn data has been imported first."
        
        response = "Here's your LinkedIn network overview: "
        response += f"You have {stats.get('totalConnections', 0)} total connections, "
        response += f"representing {stats.get('uniqueCompanies', 0)} different companies "
        response += f"across {stats.get('uniqueIndustries', 0)} industries "
        response += f"in {stats.get('uniqueLocations', 0)} locations. "
        
        if stats.get('lastUpdated'):
            response += f"Your data was last updated on {stats['lastUpdated'][:10]}."
...
I have access to your LinkedIn connections (first and second degree), uploaded contacts, and Instagram followers. I can help you find people by name, job title, company, location, or skills.

Here's what I can do:
- Search your LinkedIn network: "Find LinkedIn connections who are designers"
- General contact search: "Who do I know at Google?"
- Fetch fresh LinkedIn data: "Get my LinkedIn connections"  
- Network statistics: "Show me my LinkedIn network stats"
- Advanced searches: "Find second-degree connections in AI"

For example, you can say:
- "Find designers in my LinkedIn network"
- "Who do I know at Microsoft?" 
- "Show me software engineers in San Francisco"
- "Fetch my LinkedIn connections"
- "Get my network statistics"

When you ask me to search, I'll look through your actual data and provide real results. What would you like to do?"""
    )
