...
export class NetworkDataProcessor {
  ...
  async updateConnection(connectionId: string, updateFields: Partial<LinkedInConnection>): Promise<void> {
    try {
      ...
      const existingVector = fetchResponse.records[`linkedin_${connectionId}`];
      
      if (!existingVector) {
        throw new Error(`Connection ${connectionId} not found`);
      }

      // Merge existing metadata with updates (only simple metadata fields)
      const updatedMetadata = {
        ...existingVector.metadata,
        name: updateFields.name || existingVector.metadata?.name || '',
        title: updateFields.title || existingVector.metadata?.title || '',
        company: updateFields.company || existingVector.metadata?.company || '',
        location: updateFields.location || existingVector.metadata?.location || '',
        profileUrl: updateFields.profileUrl || existingVector.metadata?.profileUrl || '',
        industry: updateFields.industry || existingVector.metadata?.industry || '',
        summary: updateFields.summary || existingVector.metadata?.summary || '',
        skills: updateFields.skills ? JSON.stringify(updateFields.skills) : existingVector.metadata?.skills || '[]',
        lastUpdated: new Date().toISOString(),
      };
  ...
  async getNetworkStats(): Promise<NetworkStats> {
    try {
      ...
      return {
        totalConnections: stats.totalRecordCount || 0,
