...
export function UploadPanel({ onContactsUploaded, isUploading }: UploadPanelProps) {
  ...
  const parseInstagramJSON = useCallback((text: string): Contact[] => {
    try {
      ...
      let followers = [];
      if (data.instagram_followers && Array.isArray(data.instagram_followers)) {
        followers = data.instagram_followers;
      } else if (data.followers_1 && Array.isArray(data.followers_1)) {
      ...
      return followers.map((follower: any, index: number) => ({
        id: `instagram-${index}`,
        name: follower.full_name || follower.string_list_data?.[0]?.value || follower.username || follower.title || `User ${index + 1}`,
        instagram_handle: follower.username || follower.string_list_data?.[0]?.value,
        description: follower.bio || `Instagram follower`,
  ...
  const handleFileUpload = useCallback(async (file: File, type: 'linkedin' | 'instagram') => {
    console.log(`Starting file upload for ${type}:`, file.name);
    ...
    console.log(`Parsed ${contacts.length} contacts from ${type} file`);
    
    if (contacts.length > 0) {
      console.log('Sample contact:', contacts[0]);
      onContactsUploaded(contacts, type);
    } else {
      console.warn(`No contacts found in ${type} file`);
    }
    ...
    <Card className="w-full">
      <CardHeader className="text-center">
        ...
        <CardDescription className="text-base">
          Upload your LinkedIn contacts, Instagram followers, and connect your Google Calendar to start voice searching your network
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4 text-sm border border-gray-200">
          ...
          <ul className="space-y-1 text-muted-foreground">
            ...
            <li>• <strong>Google Calendar:</strong> Connect to access meeting information via voice commands</li>
